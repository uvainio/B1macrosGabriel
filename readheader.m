...
%                 event (UV)
% Edited 17.2.2009, corrected a bug: the beam size in y was the same as x (UV)

if(nargin==3) % if name is given in parts, try to guess
     name = sprintf('%s%g%s',filename,fsn,fileend);
     fid = fopen(name,'r');
     name1 = name;
     if(fid == -1) % Tries out also all filenames with zeros in between
        name = sprintf('%s0%g%s',filename,fsn,fileend);
        fid = fopen(name,'r');
     end;
     if(fid == -1)
        name = sprintf('%s00%g%s',filename,fsn,fileend);
        fid = fopen(name,'r');
     end;
     if(fid == -1)
        name = sprintf('%s000%g%s',filename,fsn,fileend);
        fid = fopen(name,'r');
     end;
     if(fid == -1)
        name = sprintf('%s0000%g%s',filename,fsn,fileend);
        fid = fopen(name,'r');
     end;
     if(fid == -1)
...
% 54, MonitorDORIS 57, Owner 58, Rot1 (x axis) 60, Rot2 (y axis) 61, PosSample 62, DetPosX 63,
...
% In case the sample name starts with number
if(strcmp(temp(1),'1') | strcmp(temp(1),'2') | strcmp(temp(1),'3') | strcmp(temp(1),'4') | strcmp(temp(1),'5') | strcmp(temp(1),'6') | strcmp(temp(1),'7') | strcmp(temp(1),'8') | strcmp(temp(1),'9'))
   temp2 = temp;
   temp = sprintf('S%s',temp2); 
end;
