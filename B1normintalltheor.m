function B1normintalltheor(fsn1,thicksfile,orifsn,fsndc,sens,errorsens,mask,energymeas,energycalib,orig)
...
%
% USES THEORETICAL TRANSMISSION FOR THE GLASSY CARBON!
%
% IN:
%
...
%              intensity
%
% Created 11.11.2007 UV (ulla.vainio@desy.de) from B1normintall.m
...
if(nargin < 9) % If initial guess is not given.
  orig = [122 124];
end;

for(m = 1:(length(emptys)-1))
  if(emptys(m+1,1) > fsn1found(emptys(m+1,2)-1)) % Process only if next file from empty is not empty
      B1normint1theor(fsn1found(emptys(m,2):(emptys(m+1,2)-1)),thicksfile,orifsn,fsndc,sens,errorsens,mask,energymeas,energycalib,orig);
  end;
end;
% And the last one separately
B1normint1theor(fsn1found(emptys(end,2):end),thicksfile,orifsn,fsndc,sens,errorsens,mask,energymeas,energycalib,orig);
