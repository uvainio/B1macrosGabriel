function B1normintallsect(fsn1,thicksfile,orifsn,fsndc,sens,errorsens,mask,energymeas,energycalib,fii1,fii2,fii3,fii4,orig)
...
% fii1      = 1st integration angle area, e.g. [0 20]
% fii2      = 2nd integration angle area, e.g. [90 110]
% fii3      = 3rd integration angle area, e.g. [180 200]
% fii4      = 4th integration angle area, e.g. [270 259]
...
if(nargin < 13) % If initial guess is not given.
  orig = [122 124];
end;

for(m = 1:(length(emptys)-1))
  if(emptys(m+1,1) > fsn1found(emptys(m+1,2)-1)) % Process only if next file from empty is not empty
      B1normint1sect(fsn1found(emptys(m,2):(emptys(m+1,2)-1)),thicksfile,orifsn,fsndc,sens,errorsens,mask,energymeas,energycalib,fii1,fii2,fii3,fii4,orig);
  end;
end;
% And the last one separately
B1normint1sect(fsn1found(emptys(end,2):end),thicksfile,orifsn,fsndc,sens,errorsens,mask,energymeas,energycalib,fii1,fii2,fii3,fii4,orig);
