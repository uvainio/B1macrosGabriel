function B1normintall(fsn1,thicksfile,orifsn,fsndc,sens,errorsens,mask,energymeas,energycalib,distminus,orig)
...
if(nargin < 11) % If initial guess is not given.
  orig = [122 124];
end;

% this line was not good:
...
% Andras Wacha, 6. Dec. 2008. (awacha@gmail.com)
...
for(m =1:(sz_emptys(1)-1))
  if(emptys(m+1,1) > fsn1found(emptys(m+1,2)-1)) % Process only if next file from empty is not empty
      B1normint1(fsn1found(emptys(m,2):(emptys(m+1,2)-1)),thicksfile,orifsn,fsndc,sens,errorsens,mask,energymeas,energycalib,distminus,orig);
  end;
end;
% And the last one separately
B1normint1(fsn1found(emptys(end,2):end),thicksfile,orifsn,fsndc,sens,errorsens,mask,energymeas,energycalib,distminus,orig);
