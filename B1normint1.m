function [qout,intout,errout,header,errmult,energyreal,distance] = B1normint1(fsn1,thicknesses,orifsn,fsndc,sens,errorsens,mask,energymeas,energycalib,distminus,orig)

% [qout,intout,errout,header] = B1normint1(fsn1,thicknesses,orifsn,fsndc,sens,errorsens,orifsn,mask,energymeas,energycalib,distminus,orig)
%
% IN:
% thicknesses = either one thickness in cm or a structure containing
% thicknesses of all samples.
% distminus = how much in mm must one subtract from the sample-to-detector distance
% 
% OUT:
%
...
% correction
       ...
       % Removed errmult from error propagation 19.2.2009
       errout(:,counter) = mult*errs(:,k)/thick.*spatialcorr.*transmcorr;       
       ...
       writelogfile(header(k),ori,thick,fsndc,energyreal(k),distance(k),mult,errmult,referencesfsn,thickGC,injectionGC,injectionEB(k),pixelsize);
